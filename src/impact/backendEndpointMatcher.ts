...
export async function matchBackendEndpoints(apiCalls: ApiCallMatch[]): Promise<BackendEndpointMatch[]> {
 ...
 for (const call of apiCalls) for (const f of javaFiles){ const text=Buffer.from(await vscode.workspace.fs.readFile(f)).toString('utf8');
  ...
  for(const m of mm){ const method=m[1].toUpperCase(); const p=(base+'/'+(m[2]||'')).replace(/\/+/g,'/'); if((call.path&&p.includes(call.path))||call.path.includes(p)){ out.push({method,path:p,controllerFile:vscode.workspace.asRelativePath(f,false),serviceCandidates:[],confidence:'CONFIRMED',evidence:['Mapeamento Spring encontrado']}); }}
 }
 return out.length?out:[{method:'UNKNOWN',path:apiCalls[0]?.path||'N/A',controllerFile:'N/A',serviceCandidates:[],confidence:'GAP',evidence:['Endpoint backend não encontrado']}];
}
